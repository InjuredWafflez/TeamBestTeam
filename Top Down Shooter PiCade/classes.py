...
class Asset(pygame.sprite.Sprite):
    def __init__(self, image = "no-image"):
        ...
        # create a collision box for the player using pygame image rectangle
...
class Player(Asset):
    ...
    def move_joystick(self, x_max, y_max):
        if(not self.game.button_map['left_up']):
            self._y_vector = -1
        
        elif (not self.game.button_map['left_down']):
            self._y_vector = 1
            
        else:
            self._y_vector = 0

        if(not self.game.button_map['left_left']):
            self._x_vector = -1
        
        elif (not self.game.button_map['left_right']):
            self._x_vector = 1
        else:
            self._x_vector = 0
...
class Weapon(Asset):
    ...
    def update(self):
        if(not self.game.button_map['right_up']):
            self._y_vector = -1
        
        elif (not self.game.button_map['right_down']):
            self._y_vector = 1
        else:
            self._y_vector = 0

        if (not self.game.button_map['right_left']):
            self._x_vector = -1
        
        elif (not self.game.button_map['right_right']):
            self._x_vector = 1
        else:
            self._x_vector = 0
    ...
    def fire(self, player):
        ...
        bullet = Bullet("images/ball.png", player, self._x_vector, self._y_vector, 1, 18)
        return bullet
...
class Weapon_Shotgun(Weapon):
    def __init__(self, image, game):
        Weapon.__init__(self, image, game)
    ...
    def fire(self, player):
        ...
        bullet1 = Bullet("images/ball.png", player, self._x_vector, self._y_vector, 1, 18)
        ...
        bullet2 = Bullet("images/ball.png", player, x_vec, y_vec, 1, 18)

        x_vec = cos(-angle)*self._x_vector - sin(-angle)*self._y_vector
        y_vec = sin(-angle)*self._x_vector + cos(-angle)*self._y_vector
        bullet3 = Bullet("images/ball.png", player, x_vec, y_vec, 1, 18)
